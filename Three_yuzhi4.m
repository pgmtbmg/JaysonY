...
% % % % % % figure
% % % % % % subplot(2,1,1);
% % % % % % plot(T_r20,X_r20);%%%%%%%未加宽度阈值
% % % % % % hold on;
% % % % % % subplot(2,1,2);
% % % % % % plot(t_r20,x_r20);%%%%加了宽度阈值
% % % % % % hold on;
% % % % % % % %
%%
% % % % % % % % % % % % % % % % % % % % % % % % 降低漏报率%%%%%%%%%%%%%%%%%%%%%%
k_ridge=fix(0.618*5);%第三个阈值，设置脊长阈值为fix(0.618*5)=3
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%加上判断，如果宽度阈值之前前三行相加等于3，而宽度阈值之后前三行相加等于二
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%则进一步判断，找出变化的位置坐标，五行加起来等于4，则保留该位置，否则去掉。
T_l3=T_R(1,:)+T_2+T_R(3,:);%T2中存放未加宽度阈值的二次分解信号,则T_l3存放未加宽度阈值时前三行的和。
T_L3=sum(T_R)-T_R(4,:)-T_R(5,:);%存放加了宽度阈值后的前三行的和。
%%%%%为确保降低漏报，进行进一步的判断
mn=(T_l3-T_L3)==1;%找出变化的向量，同时变化的位置值1，其余为0
MN=sum(mn);
if (MN>0)
    loc_1=find(mn==1);%找到变化的位置
    TL_5=sum(T_R);%加了宽度阈值的五行和
    amn=TL_5(loc_1);%找到加了宽度阈值的五行和中变化位置的向量
    a_mn=find(amn==4);%找到加了宽度阈值的五行和中变化位置的向量中有4的值的位置，这一步是避免对非故障信号的误报
    if isempty(a_mn)
        T_R(2,:)=T_R(2,:);
    else
        T_R(2,:)=T_2;
...
faulte_d=countd/count
faulte_s=counts/count
